/*
pipes working
random pipe yHeightStart also working

just keeping as in future version I'm going to make
pipeWidth, pipeHeight, birdWidth, birdHeight constant variables
instead of making them in each struct

if I ever wanted I could use this version to dynamically change
the width and height of each pipe, this could come in handier
for harder version of flappyBird, but atm I see no point
as for different widths of pipes I'd need more spirtes

could possible just do one pipe end and then pipe middle and render those
but thats too much work for something soo simple
*/
...
void drawPipeBottom(int i){
	fillRectangle(pipes[i].oldx, pipes[i].oldy + pipes[i].gap, pipes[i].width, pipes[i].height, 0);
	pipes[i].oldx = pipes[i].x;
	pipes[i].oldy = pipes[i].y;
	pipes[i].x -= pipes[i].speed;
	putImage(pipes[i].x, pipes[i].y + pipes[i].gap, pipes[i].width, pipes[i].height, pipeStart, 0, 0);
	
	int start = pipes[i].y + pipes[i].height + pipes[i].gap;
...
void activatePipes(){
	...
	int end = 100;
	for (int i = 0; i < PIPEAMT; i++) {
		if (pipes[i].active && pipes[(i + 1) % PIPEAMT].active == 0 && pipes[i].x <= (MAXSCREENX - pipes[i].spawnGap - pipes[i].width)) {
...
void gameLoop(){
	...
	while(1)
	{
		...
		if(gameCrashed == 0){
			...
			if ((bird[0].y <= MAXSCREENY-bird[0].height-1) && (bird[0].y >= 1)){
				fillRectangle(bird[0].oldx,bird[0].oldy,bird[0].width,bird[0].height,0); // x, y, width, height, colour
				bird[0].oldx = bird[0].x;
				bird[0].oldy = bird[0].y;	
			...
			if (toggle)
				putImage(bird[0].x,bird[0].y,bird[0].width,bird[0].height,bird1,0,0);
			else
				putImage(bird[0].x,bird[0].y,bird[0].width,bird[0].height,bird1,0,0);
		...
		else if (bird[0].y >= MAXSCREENY - bird[0].height){
			delay(50);
			fillRectangle(bird[0].oldx,bird[0].oldy,bird[0].width,bird[0].height,0); // x, y, width, height, colour
			putImage(bird[0].x,MAXSCREENY - bird[0].height,bird[0].width,bird[0].height,bird1,0,0);
		}
		else if (bird[0].y <= MINSCREENY){
			delay(50);
			fillRectangle(bird[0].oldx,bird[0].oldy,bird[0].width,bird[0].height,0); // x, y, width, height, colour
			putImage(bird[0].x,0,bird[0].width,bird[0].height,bird1,0,0);
		}
