...
void drawPipe(){
	...
	pipes[0].oldy = pipes[0].y;
	pipes[0].x -= pipes[0].speed;
	putImage(pipes[0].x, pipes[0].y, pipes[0].width, pipes[0].height, pipeStart, 0, 0);
	
	int start = pipes[0].y + pipes[0].height;

	for(int i = start; i <= MAXSCREENY - 1; i++){
		fillRectangle(pipes[0].oldx, i, pipes[0].width, 1, 0);
		putImage(pipes[0].x, i, pipes[0].width, 1, pipeEnd, 0, 0);
	}
...
void drawPipe2(){
	...
	fillRectangle(pipes[0].oldx, pipes[0].oldy - gap , pipes[0].width, pipes[0].height, 0);
	pipes[0].oldx = pipes[0].x;
	pipes[0].oldy = pipes[0].y;
	pipes[0].x -= pipes[0].speed;

	
	putImage(pipes[0].x, pipes[0].y - gap, pipes[0].width, pipes[0].height, pipeStart, 0, 0);
	
	int start = pipes[0].y - 1 - gap;

	for(int i = start; i >= MINSCREENY + 1; i--){
		fillRectangle(pipes[0].oldx, i, pipes[0].width, 1, 0);
		putImage(pipes[0].x, i, pipes[0].width, 1, pipeEnd, 0, 1);
	}
...
void endOfPipeCheck(){
	if (pipes[0].x < 1){
		pipes[0].x = MAXSCREENX - pipes[0].width;
		fillRectangle(MINSCREENX, MINSCREENY, pipes[0].width, MAXSCREENY, 0);
	}
}
...
// 	pipes[0].speed = 1;
// 	pipes[0].width = 20; // 20
// 	pipes[0].height = 50; // 50 (full pipe height)
// 	pipes[0].x = MAXSCREENX - pipes[0].width;
// 	pipes[0].y = MAXSCREENY - pipes[0].height;
// 	pipes[0].oldx = pipes[0].x;
// 	pipes[0].oldy = pipes[0].y;
// }
...
void initPipe2(){
	...
	pipes[0].start = 50;
	pipes[0].width = 14; // 20
	pipes[0].height = 4; // 50 (full pipe height)
	pipes[0].x = MAXSCREENX - pipes[0].width;
	pipes[0].y = MAXSCREENY - pipes[0].start;
	pipes[0].oldx = pipes[0].x;
	pipes[0].oldy = pipes[0].y;
}
